...
function navDesktopClass(pathname: string, href: string) {
...
function navMobileClass(pathname: string, href: string) {
  const active = navActive(pathname, href);
  return [navLinkMobileBase, active ? "bg-white/15 text-white" : ""].join(" ");
}
        ...
        <div className="relative border-b border-white/5 bg-black/20 backdrop-blur-md">
          <div className="mx-auto flex min-h-14 max-w-6xl items-center gap-2 px-3 py-3 sm:gap-4 sm:px-4 md:min-h-0 md:py-4 md:px-8">
            ...
            >
              ...
              <Link href="/help" className={navDesktopClass(pathname, "/help")}>
                Help
              </Link>
            </nav>
          ...
          >
            <nav className="flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pr-3 pt-3">
              <Link
                href="/play"
                onMouseEnter={prefetchLeaderboard}
                className={navMobileClass(pathname, "/play")}
              ...
              <Link
                href="/leaderboard"
                onMouseEnter={prefetchLeaderboard}
                className={navMobileClass(pathname, "/leaderboard")}
                onClick={() => setMobileMenuOpen(false)}
              >
                Hall of Fame
              </Link>
              <Link href="/help" className={navMobileClass(pathname, "/help")} onClick={() => setMobileMenuOpen(false)}>
                Help
              </Link>
            </nav>
          </div>
